...
fetch_tournament_card = (name, url, callback) ->
  ...
  request url, (error, response, body) =>
    ...
    tournament_event = $('.mod-player-stats h3').text()
    ...
    callback null, { url: url, name: name, year: year, rounds: rounds, tournament_event: tournament_event }

compact_array = (list) ->
  _.reject list, (t) -> t.length == 0

write_player_data = (name, results, callback) ->
  console.log "Writing #{tournament_event}"
  for card in results
    for round in card.rounds
      row = "#{card.url}, #{name}, #{card.year}, "
      i = 1
      ...
      fs.appendFile("event/#{tournament_event}.csv", row)
